...
function parseInjection(text) {
    const separator = text.indexOf('=');
    if (separator <= 0)
        throw new Error(`--inject expects name=value, got ${text}`);
...
function parse(argv) {
    ...
    if (inline !== undefined && codeFile !== undefined)
        throw new Error('use --code or --code-file, not both');
    ...
    if (config !== undefined) {
        ...
        if (first === undefined)
            throw new Error(`${config} has no providers[0]`);
        return {
            spec: first,
            ...code === undefined ? {} : { code },
    ...
    if (id === undefined)
        throw new Error('--id is required (or use --config)');
    const url = values.get('url')?.[0];
    const command = values.get('command')?.[0];
    if (url === undefined && command === undefined)
        throw new Error('--url (http) or --command (stdio) is required');
...
try {
    for (const provider of runtime.catalog()) {
        ...
        if (injected.length > 0)
            console.log(`  host-owned, hidden from the model: ${injected.join(', ')}`);
    }
